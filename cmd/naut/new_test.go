...
func TestScaffoldVariants(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			// scaffolding wraps `naut build`, which the SDK tier
...
func TestDeployWorkflowRenders(t *testing.T) {
	...
	for _, want := range []string{"naut build -o wf-proj", "${{ github.repository }}", "ghcr.io", "naut test ."} {
...
// their own acceptance tests — the promise `naut new` makes. This is
