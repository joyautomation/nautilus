// `naut alarms list`: dump the definitions a manifest's rules and
...
const alarmsUsage = `naut alarms — inspect a project's alarm definitions

Usage:
  naut alarms list [dir]   Expand the manifest's rules and alarm files
...
func runAlarms(args []string) int {
	...
	case "help", "--help", "-h":
		...
		return 0
	default:
		fmt.Fprintf(os.Stderr, "naut alarms: unknown command %q\n\n%s", args[0], alarmsUsage)
		return 2
	}
}
...
func runAlarmsList(args []string) int {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "naut alarms:", err)
		return 1
	}
	if proj.Alarms == nil {
		fmt.Fprintf(os.Stderr, "naut alarms: %s declares no `alarms:` section\n", manifestLabel(*manifest))
		return 1
	}
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "naut alarms: compile:", err)
		return 1
	}
