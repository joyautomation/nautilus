// `naut historian`: a standalone daemon that archives a controller's
...
// variables). naut tags are open-ended, so instead of a fixed struct
...
const historianUsage = `naut historian — archive a controller's tags into Postgres
...
Usage:
  naut historian -source <url> -db <dsn> [flags]
...
// runHistorian is the entry point for `naut historian`.
func runHistorian(args []string) int {
	...
	if *source == "" {
		fmt.Fprintln(os.Stderr, "naut historian: -source / NAUTILUS_SOURCE_URL is required")
		return 2
	}
	if *dbURL == "" {
		fmt.Fprintln(os.Stderr, "naut historian: -db / DATABASE_URL is required")
		return 2
	}
	...
	for {
		...
		fmt.Fprintf(os.Stderr, "naut historian: waiting for database: %v\n", err)
		time.Sleep(3 * time.Second)
	}
	defer store.Close()
	fmt.Fprintf(os.Stderr, "naut historian: connected to database, polling %s\n", *source)
	...
	fmt.Fprintf(os.Stderr, "naut historian: serving history on %s\n", *addr)
	if err := http.ListenAndServe(*addr, srv.handler()); err != nil {
		fmt.Fprintln(os.Stderr, "naut historian:", err)
		return 1
	}
	return 0
}
...
func collectOnce(hc *http.Client, sink hist.Sink, source string, patterns, excludes []string, filter *changeFilter, deadband float64, minInterval time.Duration) {
	...
	if err := sink.Insert(now, sample); err != nil {
		fmt.Fprintf(os.Stderr, "naut historian: insert: %v\n", err)
	}
}
	...
	for {
		...
		case <-t.C:
			if err := store.Prune(keep); err != nil {
				fmt.Fprintf(os.Stderr, "naut historian: prune: %v\n", err)
			}
		}
	}
}
