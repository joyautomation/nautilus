...
// The agent-backed half of `naut logix`.
//
...
const logixAgentUsage = `naut logix — agent-backed verbs (need a logixd agent)

  naut logix probe            Is the SDK actually usable on the agent's
                                  machine, and if not, which of the three
                                  licensing gates failed?
  naut logix agent            Agent health, open sessions, work directory.
  naut logix browse           Every controller FactoryTalk Linx can reach,
                                  with the comm path that reaches it — so no
                                  one transcribes one out of a GUI tree.
  naut logix convert <in> <out>
  ...
  naut logix build <project>  Compile the controller's routines. No
...
func reportErr(verb string, err error) int {
	fmt.Fprintf(os.Stderr, "naut logix %s: %v\n", verb, err)
	var e *logixd.Error
	if errors.As(err, &e) {
...
func runLogixBrowse(args []string) int {
	...
	fmt.Fprintf(os.Stderr, "\nPass one as --comm-path to `naut logix push` or `drift`.\n")
	return 0
}
...
func runLogixConvert(args []string) int {
	...
	if fs.NArg() != 2 {
		fmt.Fprintln(os.Stderr, "usage: naut logix convert [flags] <input> <output>")
		return 2
	}
...
func runLogixBuild(args []string) int {
	...
	if fs.NArg() != 1 {
		fmt.Fprintln(os.Stderr, "usage: naut logix build [flags] <project.ACD>")
		return 2
	}
	...
	case "echo":
		bt = logixd.BuildEcho
	default:
		fmt.Fprintln(os.Stderr, "naut logix build: --target must be default, physical or echo")
		return 2
	}
...
func runLogixPush(args []string) int {
	...
	if fs.NArg() != wantArgs || *program == "" || *routine == "" {
		fmt.Fprintln(os.Stderr,
			"usage: naut logix push [flags] <project.ACD> <rungs.L5X>   (offline)\n"+
				"       naut logix push --comm-path <path> [flags] <rungs.L5X>   (online)\n"+
	...
	if *accept && *finalize {
		fmt.Fprintln(os.Stderr, "naut logix push: --accept and --finalize are alternatives")
		return 2
	}
	...
	if opt != logixd.LeaveEdits && *commPath == "" {
		fmt.Fprintln(os.Stderr,
			"naut logix push: --accept and --finalize change a RUNNING controller, so\n"+
...
func runLogixDownload(args []string) int {
	...
	if fs.NArg() != 1 || *commPath == "" {
		fmt.Fprintln(os.Stderr, "usage: naut logix download --comm-path <path> [--program-mode] --yes <project.ACD>")
		return 2
	}
	if !*yes {
		fmt.Fprintf(os.Stderr,
			"naut logix download: refusing without --yes.\n\n"+
...
func runLogixDrift(args []string) int {
	...
	if fs.NArg() != 1 || *commPath == "" {
		fmt.Fprintln(os.Stderr, "usage: naut logix drift --comm-path <path> <repo.L5X>")
		return 2
	}
	repoPath := fs.Arg(0)
	repoRaw, err := os.ReadFile(repoPath)
	if err != nil {
		fmt.Fprintln(os.Stderr, "naut logix drift:", err)
		return 1
	}
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "naut logix drift:", repoPath, err)
		return 1
	}
	if repoFile.Partial() {
		fmt.Fprintf(os.Stderr, "naut logix drift: %s is a partial export (TargetType=%q); "+
