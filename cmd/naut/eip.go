...
const eipUsage = `naut eip — EtherNet/IP tools

Usage:
  naut eip import --host <ip> [flags]   Browse a Logix controller and
                                            generate eip_types.st + eip_manifest.go.
  naut eip browse --host <ip> [flags]   Print the controller's tag list.
  naut eip tags <eip_manifest.yaml>    Re-derive the tag file from an
...
func runEIP(args []string) int {
	...
	default:
		fmt.Fprintf(os.Stderr, "naut eip: unknown subcommand %q\n\n%s", args[0], eipUsage)
		return 2
	}
}
...
func runEIPImport(args []string) int {
	...
	if *host == "" {
		fmt.Fprintln(os.Stderr, "naut eip import: --host is required")
		return 2
	}

	ctrl, err := eipDial(*host, *slot, *port)
	if err != nil {
		fmt.Fprintln(os.Stderr, "naut eip import:", err)
		return 1
	}
	defer ctrl.Close()
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "naut eip import: browse:", err)
		return 1
	}
	...
	default:
		fmt.Fprintln(os.Stderr, "naut eip import: --format must be go or yaml")
		return 2
	}
...
func runEIPTags(args []string) int {
	...
	if fs.NArg() != 1 {
		fmt.Fprintln(os.Stderr, "usage: naut eip tags [-o tags/eip.yaml] <eip_manifest.yaml>")
		return 2
	}
	raw, err := os.ReadFile(fs.Arg(0))
	if err != nil {
		fmt.Fprintln(os.Stderr, "naut eip tags:", err)
		return 1
	}
	var m eip.Manifest
	...
	if err := dec.Decode(&m); err != nil {
		fmt.Fprintf(os.Stderr, "naut eip tags: %s: %v\n", fs.Arg(0), err)
		return 1
	}
...
func runEIPBrowse(args []string) int {
	...
	if *host == "" {
		fmt.Fprintln(os.Stderr, "naut eip browse: --host is required")
		return 2
	}
	ctrl, err := eipDial(*host, *slot, *port)
	if err != nil {
		fmt.Fprintln(os.Stderr, "naut eip browse:", err)
		return 1
	}
	defer ctrl.Close()
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
