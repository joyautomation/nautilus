...
type scaffold struct {
	...
	// `naut new` before the module is published/tagged: the generated
...
func (s *scaffold) settle() {
	...
	// `naut build` emits, which the Go tier doesn't use.
	if !s.NoGo() {
		s.Deploy = false
	}
}
...
func runNew(args []string) int {
	...
	if fs.NArg() > 0 {
		...
		// `naut new water-plant --no-input` leaves the flags unparsed.
	...
	default:
		fmt.Fprintf(os.Stderr, "naut new: --template must be one of %s\n", strings.Join(templateNames, ", "))
		return 2
	}
	switch sc.Language {
	case "", "st":
	...
	case "fbd", "ld", "sfc":
	default:
		fmt.Fprintln(os.Stderr, "naut new: --language must be st, fbd, ld, or sfc")
		return 2
	}
	if *replace != "" {
		...
		if err != nil {
			fmt.Fprintln(os.Stderr, "naut new: --replace:", err)
			return 2
		}
		sc.Replace = abs
	}
	...
	if *noInput || !stdinIsTTY() {
		...
		if sc.Name == "" {
			fmt.Fprintln(os.Stderr, "naut new: a name is required when not prompting (--no-input or no terminal)")
			return 2
		}
	} else {
		if err := prompt(&sc); err != nil {
			fmt.Fprintln(os.Stderr, "naut new:", err)
			return 1
		}
	}
	sc.settle()
  ...
  next steps:
    ...
    naut build   # emit ./%s — a self-contained controller binary
...
func prompt(sc *scaffold) error {
	...
	featureSelect := huh.NewMultiSelect[string]().
		Title("Features").
		Description("deploy applies to manifest templates (the Dockerfile wraps `naut build`)").
...
func write(sc *scaffold) error {
	...
	files := []struct {
		...
		// manifest tier: the Dockerfile wraps the binary `naut build`
	...
	if sc.Git {
		...
		if err := cmd.Run(); err != nil {
			fmt.Fprintln(os.Stderr, "naut new: git init skipped:", err)
		}
	}
	return nil
}
