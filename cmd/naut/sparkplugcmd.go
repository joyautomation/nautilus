...
// `naut sparkplug` is the import side of the Sparkplug B host
...
//
// It mirrors `naut eip import|browse|tags` deliberately, down to the flag
...
const sparkplugUsage = `naut sparkplug — Sparkplug B host application tools

Usage:
  naut sparkplug import --broker <url> --group <id> [flags]
  ...
  naut sparkplug import --sites <file> [flags]
        Generate the same files from a committed site list — no broker.
  naut sparkplug browse --broker <url> --group <id> [flags]
        Print the nodes, devices, metrics and template types discovered.
  naut sparkplug tags <sparkplug_manifest.yaml> [flags]
...
func runSparkplug(args []string) int {
	...
	default:
		fmt.Fprintf(os.Stderr, "naut sparkplug: unknown subcommand %q\n\n%s", args[0], sparkplugUsage)
		return 2
	}
}
...
func runSparkplugImport(args []string) int {
	...
	if *broker == "" && *sites == "" {
		fmt.Fprintln(os.Stderr, "naut sparkplug import: --broker or --sites is required")
		return 2
	}
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "naut sparkplug import:", err)
		return 1
	}
...
func runSparkplugBrowse(args []string) int {
	...
	if *broker == "" {
		fmt.Fprintln(os.Stderr, "naut sparkplug browse: --broker is required")
		return 2
	}
	births, err := discoverBirths(*broker, *username, *hostID, groups, *listen, *rebirth)
	if err != nil {
		fmt.Fprintln(os.Stderr, "naut sparkplug browse:", err)
		return 1
	}
	if len(births) == 0 {
...
// printBirths renders the discovery the way `naut eip browse` renders a
...
func runSparkplugTags(args []string) int {
	...
	if fs.NArg() != 1 {
		fmt.Fprintln(os.Stderr, "usage: naut sparkplug tags [--out tags/sparkplug.yaml] <sparkplug_manifest.yaml>")
		return 2
	}
	raw, err := os.ReadFile(fs.Arg(0))
	if err != nil {
		fmt.Fprintln(os.Stderr, "naut sparkplug tags:", err)
		return 1
	}
	m, err := host.ParseManifest(raw)
	if err != nil {
		fmt.Fprintf(os.Stderr, "naut sparkplug tags: %s: %v\n", fs.Arg(0), err)
		return 1
	}
