...
const logixUsage = `nautilus logix — Allen-Bradley Logix project tools
...
Usage:
  nautilus logix import <file.L5X>     Generate logix_types.st (the project's
...
func runLogixImport(args []string) int {
	...
	if fs.NArg() != 1 {
		fmt.Fprintln(os.Stderr, "usage: nautilus logix import [flags] <file.L5X>")
		return 2
	}
	f, err := l5x.ParseFile(fs.Arg(0))
	if err != nil {
		fmt.Fprintln(os.Stderr, "nautilus logix import:", err)
		return 1
	}
...
func logixTypesHeader(f *l5x.File) string {
	...
	return fmt.Sprintf(`(* Generated by `+"`nautilus logix import`"+` from the %s L5X export.
...
// runLogixGraph mirrors `nautilus ld graph`: the render model as JSON on
...
func runLogixNormalize(args []string) int {
	...
	if fs.NArg() != 1 {
		fmt.Fprintln(os.Stderr, "usage: nautilus logix normalize [flags] <file.L5X>")
		return 2
	}
	opts := l5x.NormalizeOptions{PinIDs: *pinIDs, DropL5K: *dropL5K}
	raw, err := os.ReadFile(fs.Arg(0))
	if err != nil {
		fmt.Fprintln(os.Stderr, "nautilus logix normalize:", err)
		return 1
	}
	if *check != "" {
...
func runLogixInfo(args []string) int {
	...
	if fs.NArg() != 1 {
		fmt.Fprintln(os.Stderr, "usage: nautilus logix info <file.L5X>")
		return 2
	}
	f, err := l5x.ParseFile(fs.Arg(0))
	if err != nil {
		fmt.Fprintln(os.Stderr, "nautilus logix info:", err)
		return 1
	}
	c := f.Controller
	kind := "whole project"
