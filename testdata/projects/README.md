...
- `heated-tank/` — the flagship manifest project: four tasks, three IEC
  ...
  suite (`heated-tank_test.yaml`). Cut from the former `examples/heated-tank-nogo`
  before the 2026-09 rebuild replaced it with `examples/lift-station`.
  Used by:
...
- `ladder-subroutines/` — two instances of one ladder block, each with its
  ...
  is a library like a `.st` one. Cut from the former `examples/ladder-subroutines`
  before the 2026-09 rebuild — the same feature now lives on in
  `examples/lift-station`'s `lib/motor.ld`.
  Used by:
...
- `tank-batch-sfc/program.sfc` — an SFC chart with a divergence and an
  ...
  paste) over a real chart. Cut from the former `examples/tank-batch-sfc`
  before the 2026-09 rebuild replaced it with `examples/batch-skid`.
  Used by:
