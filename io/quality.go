...
// SetQuality marks one tag's quality on the Memory driver, so a test — or a
// simulation standing in for a plant that can lose an RTU — can exercise
// the whole quality path end to end without a field bus. Setting Good
// clears the entry, which keeps Memory's report the same "non-Good only"
// shape a real driver's is.
func (m *Memory) SetQuality(name string, q Quality) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if q == Good {
		delete(m.q, name)
	...
	if m.q == nil {
		m.q = map[string]Quality{}
	}
	m.q[name] = q
}

// Quality is Memory's QualityReporter implementation: whatever SetQuality
// has been told, as a copy (the caller may hold it past the next write).
func (m *Memory) Quality() map[string]Quality {
	m.mu.Lock()
	defer m.mu.Unlock()
	if len(m.q) == 0 {
		return nil
	}
	out := make(map[string]Quality, len(m.q))
	for k, v := range m.q {
		out[k] = v
	}
	return out
...
// Memory satisfies both optional driver refinements — the point being that
// the driver every test and every loopback wiring already uses exercises
// the same paths a real field bus does.
var (
	_ BatchReader     = (*Memory)(nil)
	_ QualityReporter = (*Memory)(nil)
)
