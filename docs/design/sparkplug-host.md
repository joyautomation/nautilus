...
(`driver: {type: sparkplug-host}`) plus `naut sparkplug import`, mirroring `eip` end to end.
...
`naut eip import` gives Logix UDTs. NBIRTH `IsDefinition` templates are harvested into
...
driver:
  type: sparkplug-host
  broker: "tcp://mqtt.riverbend:1883"
  group-id: RiverbendWTP
  host-id: riverbend-central          # STATE topic spBv1.0/STATE/riverbend-central
...
```yaml
# Generated by `naut sparkplug import --broker tcp://mqtt.riverbend:1883 --group RiverbendWTP`
...
tags:
    ...
    - { name: W6_PLC1_Pump_SpeedSP, node: W6, device: PLC1, metric: Pump/SpeedSP,  type: Double,  arraylen: 0, writable: true, init: 0.0 }
      # writable → spBv1.0/RiverbendWTP/DCMD/W6/PLC1
```
...
helper `naut eip import` and `naut tags import-csv` use, so sorting, duplicate rejection,
and float formatting come free:

```yaml
# Generated by `naut sparkplug import` from births on tcp://mqtt.riverbend:1883
# (group RiverbendWTP, 2 nodes, 61 metrics). Do not edit — re-run the import.
...
**CLI.** `naut sparkplug import | browse | tags`. Import flags: `--broker` (req),
...
`TagsYAML(manifest, ...)`, so `naut sparkplug tags` needs only the repo. Follow eip's
...
2. **`New()` must not dial.** `buildDriver` runs inside `naut check` (check.go:210) and
   `naut build`, i.e. in CI with no broker. Construct offline, connect in `Start` — the eip
   split.
...
4. `cmd/naut/runcmd.go` — after line 89, add
...
7. `cmd/naut/main.go` — `case "sparkplug":` + usage line.
...
not hypothetical: the first live measurement on the Riverbend host put this block on **every** frame
...
4. **Golden-file generator test** (`cmd/naut/sparkplug_test.go`): recorded NBIRTH payloads in
...
5. **`examples/riverbend-host/`** with committed generated files, covered by
   `cmd/naut/check_manifest_test.go` — the shape regression-tested without a broker.
...
| **C2** | `cmd/naut/sparkplug.go` + `host/codegen`: import/browse/tags, `sparkplug_types.st`, golden tests | 1.5 d | ✅ |
...
| **D3** | `examples/riverbend-host/` + manifest-first guide page | 1 d | ✅ |
...
seam*; {C1, C2} are disjoint (C1: `internal/project` + schema + runcmd; C2: `cmd/naut` + a
...
8. **Verified: host-as-edge.** A project with both `driver: {type: sparkplug-host}` and a
   ...
   (twin/ 1s-scan edge on `RiverbendTwin` → site/ host-as-edge on `RiverbendWTP`, `driver:
   sparkplug-host` consuming `RiverbendTwin` primary: false → host/ central consumer of
   `RiverbendWTP`), broker `tcp://10.154.92.220:1883`:
   ...
   - **Bug found and fixed:** `rebirth-on-start: true` (the default) was a silent no-op for any
     ...
     with no OTHER primary host on it (e.g. `RiverbendTwin`, where the site is the only consumer of
   ...
   - Still open: the `naut check` warning for `driver.host-id == sparkplug.primary-host`
