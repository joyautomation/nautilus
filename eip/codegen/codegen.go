...
func Generate(br *logix.BrowseResult, opts Options) (Output, error) {
	...
	out.TypesST = renderTypesST(ordered, m, opts)
...
// stTypeName renders a manifest field/binding type in ST syntax.
func stTypeName(t string) string {
	switch t {
	case "STRING":
		return "STRING"
	case "STIME", "TIME_OF_DAY", "DATE":
		return "DINT" // ST has no direct equivalent; DINT matches the width
	case "DATE_AND_TIME":
		return "LINT"
	case "BOOL", "SINT", "USINT", "BYTE", "INT", "UINT", "WORD",
		"DINT", "UDINT", "DWORD", "LINT", "ULINT", "LWORD", "REAL", "LREAL":
		return t
	default:
		return typeIdent(t)
	}
...
// renderTypesST emits the TYPE block plus a commented VAR_EXTERNAL
// suggestion for every binding.
func renderTypesST(templates []*logix.Template, m eip.Manifest, opts Options) string {
	var sb strings.Builder
	...
	manifestTypes := map[string]eip.TypeDef{}
	for _, t := range m.Types {
		manifestTypes[t.Name] = t
	}

	if len(m.Types) > 0 {
		sb.WriteString("TYPE\n")
		for _, t := range templates {
			td, ok := manifestTypes[t.Name]
			if !ok {
				continue // string-shaped or skipped
			}
			fmt.Fprintf(&sb, "  %s : STRUCT\n", typeIdent(t.Name))
			for _, f := range td.Fields {
				if f.ArrayLen > 0 {
					fmt.Fprintf(&sb, "    %s : ARRAY [0..%d] OF %s;\n", f.Name, f.ArrayLen-1, stTypeName(f.Type))
				} else {
					fmt.Fprintf(&sb, "    %s : %s;\n", f.Name, stTypeName(f.Type))
				}
			}
	...
	for _, b := range m.Tags {
		if b.ArrayLen > 0 {
			fmt.Fprintf(&sb, "  %s : ARRAY [0..%d] OF %s;\n", b.Name, b.ArrayLen-1, stTypeName(b.Type))
		} else {
			fmt.Fprintf(&sb, "  %s : %s;\n", b.Name, stTypeName(b.Type))
		}
	}
