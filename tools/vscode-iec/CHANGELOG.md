...
- **The CLI is now `naut`.** `nautilus.cliPath` defaults to `naut`, and so do
  the in-code fallbacks every preview and the language server use. Without
  this the extension would have shelled out to `nautilus`, which on a Linux
  desktop is GNOME's file manager -- the language server would not start and
  a diagram preview would try to open a window.

  The setting is still `nautilus.cliPath` and the extension is still the
  nautilus extension; only the command changed. If you pinned the setting to
  an absolute path, nothing changes for you.
...
- **SFC: a third parallel branch joins the convergence.** *+ parallel
  ...
  (`naut sfc check` warned). The join now waits for it too, and the
...
- **The ladder commands now appear for `.L5X` files.** `Open Ladder Diagram
  ...
  the controller comparison is `naut logix drift`.
...
- **Manifest schema** learns the `modbus` driver type: `driver: {type:
  modbus}` completes and validates, with `manifest:` required (the
  imported `modbus_manifest.yaml` of sources and register bindings,
  generated by `naut modbus import`) and `scan-rate:`,
  `scan-classes:` and `tag-classes:` now documented for `modbus` as well
  as `eip`.
...
- **Acceptance tests in the Test Explorer.** `*_test.yaml` suites appear in
  VS Code's Test Explorer and run through `naut test` — per-test results,
...
- Manifest-aware language features (hover shows a tag's unit and
  ...
  position) ship with the `naut` CLI ≥ 0.5.0 — update it to light
  them up.
...
- **Full SFC comment parity with FBD/Ladder.** A "+ comment" toolbar
  ...
  **dragged to pin its layout position**, exactly like a step, addressed
  ...
  `lang/fbd` uses for its own diagram notes. `naut sfc edit` gained
...
- **Orphaned transitions are now visible and fixable from the diagram.**
  ...
  new `setTransitionEnds` structural edit op (`naut sfc edit`/
...
- **SFC (Sequential Function Chart) graphical editor** — the fourth
  ...
  the same architecture: `.sfc` text stays canonical, `naut sfc graph`
  projects the render model, and every gesture is a structural op resolved
  by `naut sfc edit` into minimal text edits.
  ...
  - No visual diff yet for `.sfc` (FBD and Ladder both have vs-HEAD/
    vs-Controller diffing) — text diff and `naut sfc check` cover chart
...
### Added
- **`pin => target` output bindings** (with the updated `naut` CLI):
...
- **B wraps the selection in a branch** (ladder): select any series
  ...
  gesture, nestable.
- `naut new --language st|fbd|ld` scaffolds a project whose program
...
- **Function contacts aren't stuck as GT.** The palette's FN( ) button
  ...
  `EQ(Step, 3)`, any function (with the updated `naut` CLI); bare
...
- **The ladder editor shares the FBD variables panel** (with the updated
  `naut` CLI): the toolbar's `vars` button now works on .ld files —
...
- **Deleting a rung's last coil no longer refuses** (with the updated
  `naut` CLI). The rung grammar needs one coil, so the old behavior
...
### Added
- **Ladder copy / cut / paste** (with the updated `naut` CLI).
...
- **Comments in the ladder diagram**, matching the FBD editor (with the
  updated `naut` CLI). Full-line `//` comment runs render as dashed
...
### Added
- **Ladder palette + drag & drop** (with the updated `naut` CLI).
...
### Added
- **Ladder editing** (with the updated `naut` CLI) — the ladder view
  is now a graphical editor, FBD-style: gestures become structural ops
  (`naut ld edit`), which rewrite the affected rung's text
...
- **Ladder Diagram (LD) — the third IEC language** (with the updated
  `naut` CLI). `.ld` files hold nautilus's textual rung format —
...
### Added
- **Arrays in FBD** (in the `naut` CLI — update it alongside the
...
- **Online edits for every task program** (with the updated `naut`
...
- **The rest of the IEC 61131-3 standard-function library** (in the
  `naut` CLI/runtime — update it alongside the extension). SEL and
...
### Added
- **Retarget a variable chip in place** (needs the updated `naut`
...
### Changed
- **Copy/paste severs wiring, keeps configuration** (in the `naut`
...
- **Variables panel.** A "vars" toolbar button lists every header
  ...
  node pills. Backed by a new `vars` list in the `naut fbd graph`
  render model.
...
- **All diagram edits are structural operations now.** A gesture no longer
  ...
  `deleteNode`) to `naut fbd edit`, which resolves it against a fresh
...
- `naut fbd edit` CLI: `{"source", "op"}` in, `{"edits"}` out — the
...
- **FBD Diagram Preview** (`nautilus: Open FBD Diagram Preview`, editor-title
  ...
  Rendering consumes `naut fbd graph` JSON, so the FBD parser exists only
  in Go.
...
- **Language server** (`naut lsp`) reusing the real `lang/st` compiler:
